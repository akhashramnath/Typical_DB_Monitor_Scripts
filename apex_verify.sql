...
--prompt V 3.5 - Added SID & APEX ADMIN related queries, 13 Aug 2018
--prompt
...
select '<body><div align=left><b><font face="Arial,Helvetica"><font color="#ff0000">' ||
       '<font size=-2>' || to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') || ' Ver 3.5 ' ||
...
--START APEX Related Schemas & Version
...
select '<h5><font face="VERDANA"><font color="#000000">ORDS Related Schemas ' ||
       ' <font size=-2></font></font></font></h5>' FROM dual;
select '<TABLE BORDER  CELLPADDING=2>' FROM dual;
select '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'ORDS Related Schemas </B></FONT></TH>' FROM dual;
...
exec dbms_output.put_line('<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>Default Schema</FONT></B></TH>');
...
col default_schema format a25
col account_locked format a1
select '<TR><TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || user_name ||'</FONT></TD>',
            '<TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || default_schema || '</FONT></TD>',
            '<TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || account_locked ||'</FONT></TD></TR>'
from &APEX..wwv_flow_fnd_user
       where user_name='ADMIN' and (default_schema='&APEX' or default_schema is NULL or default_schema ='APEX_INSTANCE_ADMIN_USER');
exec dbms_output.put_line('</TABLE>');
REM exec dbms_output.put_line('<body><font face="Arial,Helvetica"><font color="#FF0000"><font size=3><BR>If APEX Instance Administrator Account info is blank it normally means no default schema is associated with the Internal Workspace.</font></font></font></body>');
